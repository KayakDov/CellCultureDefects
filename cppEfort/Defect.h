...
public:
    ...
    /**
     * Updates the time and location to that of the st.
     * Also notes if, and how long, it's been since the last time seen.
     * @param st The current time and location.
     */
    void update(const SpaceTemp& st);
    ...
    int age() const;
private:
    int twinID = SnapDefect::NO_ID, spouseID = SnapDefect::NO_ID, timeMIA;
